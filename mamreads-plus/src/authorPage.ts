export function addMamButtonToAuthorPage() {
  const addMainAuthorButton = () => {
    ...
    const authorNameElem = document.querySelector<HTMLElement>("h1.authorName");
    ...
    if (followButtonContainer){
      // Ideal case, right under the follow button
      followButtonContainer.appendChild(mamButton);
    } else {
      // Fallback would be right next to author's name
